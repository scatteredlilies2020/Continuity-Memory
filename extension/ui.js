...
import { buildMemoryPrompt } from './retrieval.js?v=0.15.0-testing.18';
import { clearRetrievalExpansionCache, expandRetrievalTerms } from './semantic-retrieval.js?v=0.15.0-testing.18';
import { resolveRetrievalAssist } from './retrieval-assist.js?v=0.15.0-testing.18';
...
import { alignWorldToChat, collectFingerprintMessages, collectMemoryEligibleMessages, findInvalidExtractionRanges } from './message-digest.js?v=0.15.0-testing.18';
import { rankSuperiorSyncedWorlds, resolveMissingWorldBinding } from './chat-ownership.js?v=0.15.0-testing.18';
import { isRuntimeCancellation, runtime, onRuntimeChange, resumeRuntime, stopRuntime, updateRuntime } from './runtime.js?v=0.15.0-testing.18';
...
import { bindCurrentChat, getBoundWorldId, getChatKey, getSettings, markWorldDeleted, resetConfigurationSettings, resetPromptSettings, saveSettings } from './settings.js?v=0.15.0-testing.18';
import { queryEmbeddingMemory, embeddingProviderDescription, inspectEmbeddingIndex, pauseEmbeddingIndexing, purgeEmbeddingIndex, rebuildEmbeddingIndex, resumeEmbeddingIndexing, scheduleEmbeddingIndexSync, stopEmbeddingIndexing } from './embedding-retrieval.js?v=0.15.0-testing.18';
import { embeddingModelChoices, resolveEmbeddingProvider } from './embedding-provider.js?v=0.15.0-testing.18';
...
import { forkWorldToBranch } from './branch-cache.js?v=0.15.0-testing.18';
import { clampReviewFontSize, DEFAULT_REVIEW_FONT_SIZE, extractionReviewRecoveryAction, pinchedReviewFontSize, REVIEW_FONT_STEP, touchDistance } from './review-display.js?v=0.15.0-testing.18';
import { retrievalSnapshotDiagnostics } from './retrieval-snapshot.js?v=0.15.0-testing.18';
import { buildNativeChatExportRequest, readNativeChatExportResponse } from './chat-export-request.js?v=0.15.0-testing.18';
import { createRenderScheduler } from './render-scheduler.js';
import { DIRECT_CUSTOM_CHOICE, DIRECT_OPENROUTER_CHOICE, DIRECT_PROFILE_ID, directProfileChoice, parseProfileChoice } from './direct-profile.js?v=0.15.0-testing.18';
import { connectionProfileHasModel } from './profile-request-policy.js?v=0.15.0-testing.18';
...
export async function ensureCurrentChatMemory(createIfMissing = false, recoverStaleBinding = false) {
    ...
    if (boundWorldId && !recoverStaleBinding) {
        ...
        if (runtime.world?.id === boundWorldId) return runtime.world;
        try {
            return await loadBoundWorldOnce(boundWorldId);
        } catch (error) {
            if (![404, 503].includes(Number(error.status))) throw error;
            console.warn('[Continuity] Bound memory is unavailable; attempting verified chat recovery.', error);
            return await ensureCurrentChatMemory(false, true);
        }
    }
    ...
    creatingChatMemory = (async () => {
        ...
        if (portable && !(getSettings().deletedWorldIds || []).includes(portable.world.id)) {
            ...
            if (existing?.corrupt && !boundElsewhere) {
                ...
                toast('success', 'Recovered this chat’s Continuity memory from its verified embedded copy.');
        ...
        while (true) {
            ...
            try {
                ...
                if (failures >= maxFailures) {
                    const message = `Build stopped after ${failures} temporary failures to limit repeated API requests. Saved progress was kept.`;
                    updateRuntime({ status: 'error', lastError: error.message, retryStatus: message });
...
async function undoLatestDigestMemory() {
    ...
    if (!latest.replayable) throw new Error(LEGACY_DIGEST_RESCAN_MESSAGE);
    const dependent = [
