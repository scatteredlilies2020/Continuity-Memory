...
import { alignWorldToChat, collectFingerprintMessages, collectMemoryEligibleMessages } from './message-digest.js?v=0.14.0-standalone.250';
import { resolveMissingWorldBinding } from './chat-ownership.js?v=0.14.0-standalone.250';
import { isRuntimeCancellation, runtime, onRuntimeChange, resumeRuntime, STORY_RUNTIME_STATUSES, stopRuntime, stopRuntimeTask, updateRuntime } from './runtime.js?v=0.14.0-standalone.250';
...
import { bindCurrentChat, getBoundWorldId, getChatKey, getSettings, markWorldDeleted, resetConfigurationSettings, resetPromptSettings, saveSettings } from './settings.js?v=0.14.0-standalone.250';
import { embeddingProviderDescription, pauseEmbeddingIndexing, purgeEmbeddingIndex, rebuildEmbeddingIndex, resumeEmbeddingIndexing, scheduleEmbeddingIndexSync, stopEmbeddingIndexing } from './embedding-retrieval.js?v=0.14.0-standalone.250';
import { embeddingModelChoices, resolveEmbeddingProvider } from './embedding-provider.js?v=0.14.0-standalone.250';
...
import { forkWorldToBranch } from './branch-cache.js?v=0.14.0-standalone.250';
import { clampReviewFontSize, DEFAULT_REVIEW_FONT_SIZE, extractionReviewRecoveryAction, pinchedReviewFontSize, REVIEW_FONT_STEP, touchDistance } from './review-display.js?v=0.14.0-standalone.250';
import { retrievalSnapshotDiagnostics } from './retrieval-snapshot.js?v=0.14.0-standalone.250';
import { resolveStoryBudget } from './story-budget.js?v=0.14.0-standalone.250';
import { resolveStoryBatchMessages, rollingStoryCoverage, stableStoryMessages } from './story-cadence.js?v=0.14.0-standalone.250';
import { resolveStorySourceMode, storedStorySourceMode, storySourceModeLabel, storySourcePolicyIsCurrent, STORY_SOURCE_L1 } from './story-source.js?v=0.14.0-standalone.250';
import { createRenderScheduler } from './render-scheduler.js';
import { DIRECT_CUSTOM_CHOICE, DIRECT_OPENROUTER_CHOICE, DIRECT_PROFILE_ID, directProfileChoice, parseProfileChoice } from './direct-profile.js?v=0.14.0-standalone.250';
import { connectionProfileHasModel } from './profile-request-policy.js?v=0.14.0-standalone.250';
...
function renderMemoryViewer(force = false) {
    ...
    const result = memoryViewerPage(world, viewerCategory, viewerSearch, viewerPage, 30, getChatKey());
...
export function renderRuntime(refreshSettings = true) {
    ...
    const storyEligibleMessages = storySourceMode === STORY_SOURCE_L1
        ...
        : stableMessages;
    const storyCoverage = rollingStoryCoverage(rollingStory, storyEligibleMessages);
    const storySourceChanged = Boolean(rollingStory?.text)
        && (storedStorySourceMode(rollingStory) !== storySourceMode || !storySourcePolicyIsCurrent(rollingStory, storySourceMode));
    ...
    $('#continuity_story_progress')
        ...
        : String(runtime.storyStatus).startsWith('pending-story-')
            ...
            : runtime.storyStatus === 'stopping'
                ? runtime.storyRetryStatus || 'Stopping Story build safely…'
                : storyProgress?.total
                    ? `Processing Story chunk ${storyProgress.current}/${storyProgress.total} · messages ${Number(storyProgress.from ?? 0) + 1}–${Number(storyProgress.to ?? 0) + 1} · ~${storyProgress.inputTokens || '?'} source tokens`
                    : runtime.storyRetryStatus || 'Story build is processing independently…');
    $('#continuity_story_progress_bar')
        .toggle(storyProcessing)
        .attr('max', Math.max(1, Number(storyProgress?.total) || 1));
    ...
    $('#continuity_story_status').text(storySourceChanged
        ...
        : rollingStory?.text
            ? `${storyCoverage.current ? `Story so far is already current through message ${storyCoverage.through + 1}` : `Stored through message ${storyCoverage.through + 1}; ${storyCoverage.pending} eligible message(s) pending`}; approximately ${Math.ceil(String(rollingStory.text).length / 4)} tokens before injection clipping. Current ${resolvedStoryAllowance.mode} allowance: ${resolvedStoryAllowance.tokens} tokens.`
    ...
    const storyText = runtime.storyProcessing ? ` · Story ${runtime.storyStatus}` : '';
    $('#continuity_status').text(`${runtime.paused ? 'Paused' : runtime.status}${queueText}${storyText}`);
    const progress = runtime.progress;
    $('#continuity_progress').text(progress?.phase === 'pending'
        ? `Pending: ${progress.label || 'preparing work'} · messages ${Number(progress.from ?? 0) + 1}–${Number(progress.to ?? 0) + 1}`
        : progress
    ...
    const diagnostic = {
        ...
        storyEngine: runtime.storyStatus,
        storyProcessing: runtime.storyProcessing,
        storyProgress: runtime.storyProgress,
        storyLastError: runtime.storyLastError,
...
// Rendering at most ten times per second keeps the settings panel responsive.
const scheduleRuntimeRender = createRenderScheduler(() => renderRuntime(false), undefined, { minInterval: 100 });
...
async function detachToEmptyMemory({ confirm = true } = {}) {
    ...
    if (runtime.processing || runtime.storyProcessing) throw new Error('Stop memory and Story processing before detaching memory.');
...
async function deleteScope() {
    ...
    if (runtime.processing || runtime.storyProcessing) throw new Error('Stop memory and Story processing before deleting memory.');
...
export function initUI() {
    ...
    $('#continuity_story_build').on('click', async () => {
        ...
        const messageCount = eligibleMessages.length;
        if (!messageCount) return toast('error', 'This chat has no eligible raw messages to summarize.');
        const targetTo = Number(stored?.rebuildTargetTo);
        const savedTo = Number(stored?.to ?? -1);
        const pending = stored?.rebuildRestartPending
            ? eligibleMessages.filter(message => !Number.isFinite(targetTo) || Number(message.index) <= targetTo).length
    ...
    $('#continuity_story_stop').on('click', () => {
        const stopped = stopRuntimeTask(STORY_RUNTIME_STATUSES, 'Stopping Story so far; completed batches remain saved and the in-flight batch will be discarded.');
