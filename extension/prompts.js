...
Return exactly four arrays in the requested JSON object:
...
The field limits are hard editorial budgets, not targets. Allocate entries by durable explanatory value, never recency, position, message count, scene length, drama, prose intensity, or available space. The newest excerpt has no reserved share. A completed scene gets at most one majorDevelopments entry unless it independently creates two irreversible transformations. Returning fewer entries is preferred.

Every entry must pass this counterfactual test: removing it would make a later identity, role, relationship, motive, conflict, commitment, rule, knowledge boundary, consequence, or unresolved matter materially harder to understand. Otherwise omit it. Collapse action into result: who changed what, its durable consequence, and why it matters. Generalize or omit exact counts, percentages, durations, colors, materials, measurements, minor actors, staff rosters, vehicles, unit names, equipment specifications, route or zone codes, games, routine reassurance, temporary injuries or treatment, and combat techniques. Preserve an exact name, value, object, deadline, or rule only when exactness itself drives later causality. Never inventory possessions or repeat one fact across sections.

Use one dense telegraphic sentence or fragment per array entry. Omit headings, markdown, atmosphere, sensory detail, scene-setting, dialogue retelling, transitions, literary phrasing, repetition, and internal commentary. The compiled snapshot's token allowance is a ceiling, never a target. Compress wording, never causal meaning.
...
export const ROLLING_STORY_TASK_TEMPLATE = `Rewrite the complete world-state snapshot through the end of this raw excerpt. The compiled snapshot must not exceed {{allowance}} tokens.
