...
export const RELATIONAL_ADDRESS_RULE = `When actors communicate, store distinctive forms of address, including honorifics, titles, nicknames, callsigns, and first-name use, as a separate persistent fact for each established speaker-to-addressee pattern. Use the canonical speaker as subject, "form of address for [canonical addressee]" as predicate, and exact current plus meaningful former forms as value. Update a relationship only when an address shift signals changed familiarity, distance, respect, or hierarchy. Ignore ordinary one-off usage.`;
