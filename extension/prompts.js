...
export const RELATIONSHIP_DESCRIPTION_RULE = `Each unordered participant pair has one canonical relationship record. from and to identify the participants only; their order assigns no role or direction. Reuse the supplied relationship targetId when the same pair reappears, even if the endpoints reverse or its wording evolves, and combine simultaneous roles or dependencies into that record instead of creating variants. Keep kind a short stable type label. Make dynamic the authoritative self-contained description: explicitly name each participant, state every asymmetric role or dependency, and preserve the current interaction pattern and meaningful change. Never rely on endpoint order or the order of words in kind to identify who holds a role.`;
...
export const IDENTITY_RESOLUTION_RULES = `Use identityResolutions only when the narrative establishes that an earlier descriptive, unknown, disguised, or aliased reference is a canonical entity present in context. Give one short evidence sentence. Never resolve one established named actor into another, and never treat a possessive object (someone's weapon, clothing, vehicle, remains, record, or proof) as that owner. Never resolve from outside knowledge, genre convention, resemblance, suspicion, prediction, or an unconfirmed claim; retain useful uncertainty as a claim, belief, or thread. Otherwise leave identityResolutions empty.`;
