...
export const ROLLING_STORY_RULE = `storySoFar is a separate retrospective rolling narrative, not a memory record. Rewrite the complete summary from the supplied prior story plus the new raw excerpt only. Preserve the causal path from the beginning through the end of the excerpt, including major decisions, changes, consequences, and unresolved matters, while compacting older detail lossily. Narrate only what has already occurred in the supplied material. Never call a condition current, present, latest, or ongoing, and never use currently, now, at present, or similar framing. Do not predict or narrate future events. If the source contains a plan, intention, promise, expectation, or unresolved goal, report only that it was expressed or left unresolved; do not present its anticipated outcome as having occurred. Do not consult, copy, cite, or name L1, L2, L3, retrieved records, hierarchy levels, memory categories, or internal IDs. Use natural prose and explicit names. If there is no prior story, begin from the raw excerpt.`;
