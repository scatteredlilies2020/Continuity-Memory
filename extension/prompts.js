...
export const CANONICAL_RECORD_RULES = `Canonical memory context contains relevant existing mutable records with stable targetId values. When the excerpt changes an existing entity, fact, state, relationship, or thread, return the updated record with that exact targetId and preserve its canonical name, predicate, attribute, relationship kind, or thread title. Omit an existing record when the excerpt only repeats it unchanged. Leave targetId empty only for genuinely new information. Use recordMerges only when supplied records clearly describe the same durable fact, state, relationship, or thread; select one canonicalId, list the redundant duplicateIds, and give one short evidence sentence. Never merge distinct goals, simultaneous values, similar relationships, recurring actions, or separate event occurrences. Apply this generically to the scenario's own ontology and wording.`;

export const CONTINUITY_COVERAGE_RULES = `Coverage and retrieval are separate concerns. Before returning JSON, silently inventory every distinct continuity-bearing strand in the excerpt, including side characters, subplots, locations, factions, operations, offscreen developments, reports, and background processes. If a strand meaningfully changes, completes an event, establishes a durable detail, or remains unresolved with plausible future relevance, preserve it in at least one appropriate structured record. Do not omit a strand merely because it is outside the focal scene or less important than another plot. The sceneCapsule should stay concise and follow the main chronological or causal spine; background coverage belongs in compact facts, states, relationships, events, or threads rather than being forced into the capsule. Omit pure atmosphere, repetition, unchanged decorative status, and details with no plausible continuity value. When uncertain about future relevance, retain one compact low-importance record. Never combine unrelated strands merely to save space.`;
