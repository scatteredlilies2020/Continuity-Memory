...
export const ROLLING_STORY_RULE = `storySoFar is the global causal spine of the supplied history, not a scene recap or memory record. Rewrite the complete digest from the prior story plus the new raw excerpt only. Re-rank the whole history on every call; never append merely because material is new. Returning the prior story unchanged is preferred when the excerpt adds no load-bearing development.

Silently map the history into causal phases before writing. Open with the earliest premise, origin, or initiating cause still needed to understand everything later, then follow strict causal chronology. Allocate space by future explanatory value, not recency, position, message count, scene length, drama, prose intensity, or available token budget. Preserve origins, irreversible transformations, identity or role changes, central relationships and commitments, decisive knowledge changes, durable constraints, major consequences, and unresolved central conflicts until superseded. Use saved space to cover more of the causal span, never to add texture.

Prevent scene capture. A completed scene normally receives one compact outcome-and-consequence clause; only a premise-defining or irreversible transformation may need a second. When the history has at least five causal phases, no one completed scene or newest excerpt should occupy more than about one fifth of the digest unless it alone fundamentally changes the premise. The newest excerpt has no reserved share. Collapse action into result: who confronted whom, what materially changed, and why it matters later. Do not retell how each move, conversation, journey, recovery, briefing, or routine interaction unfolded.

Every retained detail must pass this test: removing it would make a later identity, role, relationship, motive, conflict, commitment, rule, knowledge boundary, consequence, or unresolved matter materially harder to understand. Otherwise delete it. A calm discovery or decision may pass; affection, travel, rest, logistics, repeated sentiment, games, ordinary reassurance, temporary injury or treatment, visual design, and tactical execution usually do not. Generalize exact counts, percentages, durations, colors, materials, measurements, minor names, supporting-cast rosters, vehicle or unit names, equipment specifications, route or zone codes, and combat techniques. Keep an exact name, value, object, deadline, or rule only when its exactness drives later causality. Never inventory.

Use dense telegraphic prose, compact fragments, and semicolon-linked clauses. Omit headings, bullets, formatting, atmosphere, scene-setting, dialogue retelling, transitions, repetition, literary phrasing, and duplicate end-state lists. Make the digest as short as possible without losing its load-bearing causal chain; the token allowance is a ceiling, never a target. Compress wording, never causal meaning.

Record only what has already occurred in the supplied material. Never call a condition current, present, latest, or ongoing, and never use currently, now, at present, or similar framing. Do not predict or narrate future events. Report a plan, intention, promise, expectation, or unresolved goal only as expressed or unresolved, never as completed. Attribute beliefs, reports, deception, and uncertainty to their holders or sources. Use explicit names only where omission would create ambiguity. Do not consult, copy, cite, or name L1, L2, L3, retrieved records, hierarchy levels, memory categories, or internal IDs. If there is no prior story, begin from the raw excerpt.

Before returning, silently verify that the opening contains the foundational cause, the newest scene has not displaced older causal coverage, each completed scene is outcome-level rather than action-level, and every retained number, named object, minor actor, or physical specification is indispensable.`;

export const ROLLING_STORY_TASK_TEMPLATE = `Rewrite the rolling story through the end of this raw excerpt. storySoFar must not exceed {{allowance}} tokens.
First perform the silent causal-phase and detail-necessity checks required by the system instruction. Return only the complete rewritten digest in the requested JSON shape; do not expose analysis.
{{format}}
