...
import { collectFingerprintMessages, collectMemoryEligibleMessages, findChangedExtractions, fingerprintMessage } from './message-digest.js?v=0.14.0-standalone.186';
...
import { isExplicitExtractionOutputLimitError, processAdaptiveExtractionChunks } from './extraction-recovery.js?v=0.14.0-standalone.186';
...
import { outputTokenPayload } from './model-compatibility.js?v=0.14.0-standalone.186';
import { formatExtractionMessages, precedingUserAttributionContext } from './extraction-context.js?v=0.14.0-standalone.186';
import { embedWorldInChat } from './portable.js';
import { isolatedProfileOptions, isolatedProfilePayload } from './profile-request-policy.js?v=0.14.0-standalone.186';
...
import { getBoundWorldId, getChatKey, getSettings } from './settings.js?v=0.14.0-standalone.186';
import { buildThinkingRequest, isThinkingControlError, shouldSendStructuredSchema } from './thinking-policy.js?v=0.14.0-standalone.186';
import { onRuntimeStop, runtime, updateRuntime } from './runtime.js?v=0.14.0-standalone.186';
            ...
            type: 'array', items: {
                ...
                properties: {
                    ...
                    characterProfile: {
                        ...
                        properties: {
                            roleBackground: { type: 'array', maxItems: 8, items: { type: 'string' }, description: 'Atomic established roles, identity-defining background details, and durable social functions for this recurring named person. Ground in narrative or accepted memory, never status/control-panel fields. Empty when unknown or not a person.' },
                            appearance: { type: 'array', maxItems: 8, items: { type: 'string' }, description: 'Atomic durable observable appearance details for this recurring named person. Ground in narrative or accepted memory, never status/control-panel fields. Exclude temporary clothing, wounds, emotion, and pose.' },
                            personalityQuirks: { type: 'array', maxItems: 8, items: { type: 'string' }, description: 'Atomic established recurring temperament, habits, speech patterns, and quirks for this recurring named person. Ground in narrative or accepted memory, never status/control-panel fields. Exclude one-off reactions.' },
                        },
                    },
                    importance: { type: 'integer', minimum: 1, maximum: 5 },
                },
...
Do not alter unrelated details, invent unsupported events, create new L1 capsules, or edit chat messages. ${NO_EM_DASH_STYLE_RULE} Return JSON only.`;
...
export async function loadBoundWorld() {
    ...
    const compactedDuplicateRecords = compactDuplicateMemoryRecords(world) > 0;
