...
import { collectFingerprintMessages, collectMemoryEligibleMessages, findChangedExtractions, fingerprintMessage } from './message-digest.js?v=0.14.0-standalone.301';
...
import { isExplicitExtractionOutputLimitError, processAdaptiveExtractionChunks } from './extraction-recovery.js?v=0.14.0-standalone.301';
...
import { outputTokenPayload } from './model-compatibility.js?v=0.14.0-standalone.301';
import { formatExtractionMessages, precedingUserAttributionContext } from './extraction-context.js?v=0.14.0-standalone.301';
import { embedWorldInChat } from './portable.js';
import { connectionProfileModel, isolatedProfileOptions, isolatedProfilePayload } from './profile-request-policy.js?v=0.14.0-standalone.301';
...
import { applySourceAttributionFailClosed, canonicalFactReference, sanitizeReconciliationMetadata } from './reconciliation-policy.js?v=0.14.0-standalone.301';
import { getBoundWorldId, getChatKey, getSettings } from './settings.js?v=0.14.0-standalone.301';
import { buildThinkingRequest, isMandatoryThinkingError, isThinkingControlError, mandatoryThinkingPayload, shouldSendStructuredSchema, thinkingControlFallbackPayload } from './thinking-policy.js?v=0.14.0-standalone.301';
import { isRuntimeCancellation, onRuntimeStop, resumeRuntime, RUNTIME_CANCELLED_CODE, runtime, updateRuntime } from './runtime.js?v=0.14.0-standalone.301';
import { completedDetachedWorldIsNewer, detachedProgressNeedsRefresh, latestCompletedDetachedJob } from './detached-reconnect-policy.js?v=0.14.0-standalone.301';
import { isActiveState, latestSourceRange } from './state-lifecycle.js';
import { temporalContext } from './temporal-anchors.js';
import { dynamicStoryRefineSourceChunk, dynamicStorySourceChunk, resolveStoryBudget, storyWithinAllowance } from './story-budget.js?v=0.14.0-standalone.301';
import { storyCompressionTarget, storyGenerationTargets } from './story-output-policy.js?v=0.14.0-standalone.301';
import { resolveStoryRequestProfile } from './story-profile.js?v=0.14.0-standalone.301';
import { resolveProfileThinkingMode } from './story-thinking.js?v=0.14.0-standalone.301';
import { alignStoryRebuildTarget, completeStoryMessages, resolveStoryBatchMessages, rollingStoryBuildPlan, rollingStoryRebuildCheckpoint, rollingStoryRebuildPlan, stableStoryMessages, storyChunkMessageLimit } from './story-cadence.js?v=0.14.0-standalone.301';
import { compileRollingStorySnapshot, ROLLING_STORY_SNAPSHOT_EXAMPLE, ROLLING_STORY_SNAPSHOT_SCHEMA } from './story-snapshot.js?v=0.14.0-standalone.301';
import { planStoryMutationRecovery, withStoryCheckpoint } from './story-checkpoints.js?v=0.14.0-standalone.301';
import { buildStorySourceUnits, isCurrentStorySnapshot, resolveStorySourceMode, storedStorySourceMode, storySourceModeLabel, storySourcePolicyIsCurrent, STORY_FORMAT_MANUAL, STORY_SOURCE_L1, STORY_SOURCE_POLICY_VERSION } from './story-source.js?v=0.14.0-standalone.301';
import { DIRECT_PROFILE_ID } from './direct-profile.js?v=0.14.0-standalone.301';
...
const rollingStoryQualityReviewJsonSchema = Object.freeze({
    name: 'continuity_rolling_story_quality_review',
    description: 'A strict factual and compression review of a repaired rolling Story candidate.',
    strict: true,
    returnInvalid: true,
    value: {
        type: 'object',
...
async function verifyRollingStoryQuality(candidate, priorStory, messages, {
    ...
    const usesStructuredSchema = requestSupportsStructuredSchema(rollingStoryQualityReviewJsonSchema, profileId, directKind, thinkingMode);
    const format = usesStructuredSchema
    ...
    const raw = await requestStructured(
        ...
        rollingStoryQualityReviewJsonSchema,
        storyResponseTokens(),
        profileId,
        directKind,
        fallbackPrompt,
    ...
    if (typeof parsed?.valid !== 'boolean' || !Array.isArray(parsed?.issues)) {
        throw new Error('The Story quality verifier returned invalid JSON.');
    }
    return {
        valid: parsed.valid && parsed.issues.length === 0,
        issues: parsed.issues.map(item => String(item || '').trim()).filter(Boolean).slice(0, 12),
    };
}
...
async function repairRollingStoryQuality(candidate, priorStory, messages, {
    ...
    const values = {
        ...
        messages: formatExtractionMessages(messages),
        format,
        feedback: String(initialFeedback || '').trim() || '(No previous verifier feedback.)',
    };
    let lastError;
    let feedback = values.feedback;
    for (let attempt = 1; attempt <= 3; attempt++) {
        if (runtime.storyGeneration !== expectedEpoch) {
            const error = new Error('Story quality repair was stopped.');
        ...
        try {
            ...
            const raw = await requestStructured(
                ...
                profileId,
                directKind,
                `${fallbackPrompt}${retry}`,
                thinkingMode,
            );
            const parsed = typeof raw === 'string' ? parseJsonResponse(raw) : raw;
            const repaired = compileRollingStorySnapshot(parsed?.storySoFar ?? parsed);
            if (!repaired) throw new Error('The summarizer returned an empty rolling story during quality repair.');
            const measuredTokens = Math.max(1, Number(await getTokenCountAsync(repaired)) || Math.ceil([...repaired].length / 4));
            ...
            if (!review.valid) {
                feedback = review.issues.length
                    ...
                    : 'The verifier rejected the candidate without details. Recheck exact learned names, hidden identity links, active ultimata, severity, and inventories.';
                lastError = new Error(`Story quality verifier rejected the repaired snapshot: ${feedback.replace(/\s+/gu, ' ').trim()}`);
                if (attempt >= 3) throw lastError;
                updateRuntime({ storyRetryStatus: `Story quality verifier found ${review.issues.length || 1} issue(s); retrying the repair before saving.` });
                continue;
            }
            return repaired;
        } catch (error) {
            lastError = error;
            if (runtime.storyGeneration !== expectedEpoch || isRuntimeCancellation(error)) throw error;
            if (attempt >= 3 || !storyRetryableError(error)) throw error;
            updateRuntime({ storyRetryStatus: `Final Story quality repair attempt ${attempt}/3 failed temporarily; retrying without saving the unchecked draft.` });
        }
    }
    throw lastError;
}
...
export async function refineRollingStory() {
    if (runtime.storyProcessing) throw new Error('A Story operation is already running.');
    const worldId = getBoundWorldId();
    const chatKey = getChatKey();
    if (!worldId || !chatKey) throw new Error('Open a chat with Continuity memory first.');
    ...
    updateRuntime({
        ...
        storyStatus: 'refining-story',
        storyLastError: '',
        storyFailure: null,
        storyProgress: { phase: 'pending', label: 'preparing Story refinement' },
        storyRetryStatus: 'Preparing the stored Story and its covered source for manual refinement…',
    });
    try {
        await requireRetryStorage();
        world = world || (await api.getWorld(worldId)).world;
        previous = world.storySoFar?.[chatKey];
        if (!String(previous?.text || '').trim()) throw new Error('Build Story so far before refining it.');
        if (previous.rebuildIncomplete || previous.rebuildRestartPending) {
            throw new Error('Finish the pending Story build before refining it.');
        }
        const settings = getSettings();
        const sourceMode = resolveStorySourceMode(settings.storySourceMode);
        if (storedStorySourceMode(previous) !== sourceMode || !storySourcePolicyIsCurrent(previous, sourceMode)) {
            throw new Error('The stored Story uses a different source policy. Build / Continue or Rebuild it before refining.');
        }
        const allMessages = stableStoryMessages(collectMemoryEligibleMessages(getContext().chat || []));
        const through = Number(previous.to);
        const coveredMessages = allMessages.filter(message => Number(message.index) <= through);
        ...
        const allowance = resolveStoryBudget(settings.storySoFarTokens, getContext().maxContext).tokens;
        const sourceLimit = dynamicStoryRefineSourceChunk(getContext().maxContext, allowance);
        chunks = await chunkMessages(sourceBreakdown.units, sourceLimit, Infinity, sourceLimit);
        if (runtime.storyGeneration !== epoch) {
            const error = new Error('Story refinement generation was stopped while preparing its source.');
            error.code = RUNTIME_CANCELLED_CODE;
            throw error;
        }
        const { profileId, directKind } = resolveStoryRequestProfile(settings, DIRECT_PROFILE_ID);
        const thinkingMode = resolveThinkingModeForProfile(settings.storyThinkingMode, profileId);
        const usesStructuredSchema = requestSupportsStructuredSchema(rollingStoryJsonSchema, profileId, directKind, thinkingMode);
        const characterBudget = storyGenerationTargets(allowance).characterBudget;
        let story = String(previous.text).trim();
        let repairedChunks = 0;
        let foundIssues = 0;
        for (let index = 0; index < chunks.length; index++) {
            if (runtime.storyGeneration !== epoch) {
                const error = new Error('Story refinement generation was stopped; the stored Story was left unchanged.');
                error.code = RUNTIME_CANCELLED_CODE;
                throw error;
            }
            ...
            updateRuntime({
                ...
                storyRetryStatus: `Reviewing Story refinement source ${index + 1}/${chunks.length}…`,
            });
            const review = await verifyRollingStoryQuality(story, story, chunk.messages, { profileId, directKind, thinkingMode });
            if (runtime.storyGeneration !== epoch) {
                const error = new Error('Story refinement generation was stopped; the stored Story was left unchanged.');
                error.code = RUNTIME_CANCELLED_CODE;
                throw error;
            ...
            foundIssues += review.issues.length || 1;
            const feedback = review.issues.length
                ? review.issues.map((issue, issueIndex) => `${issueIndex + 1}. ${issue}`).join('\n')
                : 'The verifier rejected the candidate without details. Recheck exact learned names, hidden identity links, active ultimata, severity, and inventories.';
            updateRuntime({ storyRetryStatus: `Refining Story from ${review.issues.length || 1} issue(s) found in source ${index + 1}/${chunks.length}…` });
            story = await repairRollingStoryQuality(story, story, chunk.messages, {
                allowance,
                characterBudget,
                profileId,
                directKind,
        ...
        if (story === String(previous.text).trim()) {
            updateRuntime({ storyStatus: 'idle', storyProgress: null, storyFailure: null, storyRetryStatus: `Refine complete: no material correction was needed across ${chunks.length} source chunk(s).` });
            return { world, changed: false, issues: foundIssues, repairedChunks, chunks: chunks.length };
        }
        const refined = withStoryCheckpoint(previous, {
            ...previous,
            text: story,
            updatedAt: new Date().toISOString(),
        }, allMessages, fingerprintMessage);
        world = await persistRollingStory(worldId, chatKey, refined);
        await embedWorldInChat(world);
        updateRuntime({ storyStatus: 'idle', storyProgress: null, storyFailure: null, storyRetryStatus: `Refine complete: repaired ${repairedChunks} source chunk(s) and saved the revised Story.` });
        return { world, changed: true, issues: foundIssues, repairedChunks, chunks: chunks.length };
    } catch (error) {
        if (isRuntimeCancellation(error)) {
            updateRuntime({ storyStatus: 'idle', storyProgress: null, storyLastError: '', storyFailure: null, storyRetryStatus: error.message });
        } else {
            const recovery = 'The stored Story was left unchanged; Refine can be retried.';
            updateRuntime({
                storyStatus: 'error',
                storyProgress: null,
                storyLastError: error.message,
                storyFailure: { chatKey, message: error.message, recovery, at: new Date().toISOString() },
                storyRetryStatus: `Story refinement failed safely: ${error.message} ${recovery}`,
            });
        }
        throw error;
    } finally {
...
async function runManualRollingStory(rebuildFromBeginning) {
    ...
    try {
        ...
        plan = rebuildFromBeginning || sourceChanged
            ? rollingStoryRebuildPlan(availableMessages)
            : rollingStoryBuildPlan(availableMessages, previous);
...
export async function maybeAutoUpdateRollingStory(sourceMessages = null) {
    ...
    const sourceMode = resolveStorySourceMode(settings.storySourceMode);
    if (previous?.text && (storedStorySourceMode(previous) !== sourceMode || !storySourcePolicyIsCurrent(previous, sourceMode))) {
    ...
    const firstIndex = Number(previous?.from ?? availableMessages[0]?.index ?? ready[0].index);
    updateRuntime({
        storyProcessing: true,
...
export async function repairDivergedBranch({ sourceMessages = null, sourceMutation = false } = {}) {
    if (runtime.processing) return { deferred: true, repaired: false };
    const worldId = getBoundWorldId();
    const chatKey = getChatKey();
    if (!worldId || !chatKey) throw new Error('Open a chat with Continuity memory first.');
    await requireRetryStorage();
