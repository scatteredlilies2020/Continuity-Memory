...
import { api } from './api.js?v=0.15.0-testing.22';
import { captureChatCompletionOverhead, captureTextCompletionOverhead, reduceChatContext } from './context-reducer.js';
import { applyExtractionRequestSettings, getProcessingCoverage, getTailRollbackStatus, loadBoundWorld, maintainChronicleHierarchy, maybeAutoExtract, repairDivergedBranch, syncChangedExtractions } from './engine.js?v=0.15.0-testing.22';
import { buildMemoryPrompt, prepareRetrievalCorpus } from './retrieval.js?v=0.15.0-testing.22';
import { invalidateRuntimeWork, invalidateStoryWork, isRuntimeCancellation, onRuntimeChange, onRuntimeStop, resumeRuntime, runtime, updateRuntime } from './runtime.js?v=0.15.0-testing.22';
import { getBoundWorldId, getChatKey, getSettings, saveSettings } from './settings.js?v=0.15.0-testing.22';
import { ensureCurrentChatMemory, initUI, refreshModelProfiles, renderRuntime, refreshWorlds, restorePendingExtractionReview } from './ui.js?v=0.15.0-testing.22';
...
import { resolveDeletedChatBinding, resolveRenamedChatBinding } from './chat-ownership.js?v=0.15.0-testing.22';
import { collectFingerprintMessages, collectMemoryEligibleMessages, findInvalidExtractionRanges } from './message-digest.js?v=0.15.0-testing.22';
import { queryEmbeddingMemory, purgeEmbeddingIndex, scheduleEmbeddingIndexSync } from './embedding-retrieval.js?v=0.15.0-testing.22';
import { isTransientApiError } from './errors.js?v=0.15.0-testing.22';
import { roleplaySourceMessages, shouldGateRoleplayGeneration, sourceMutationPolicy } from './generation-policy.js?v=0.15.0-testing.22';
...
import { createRetrievalSnapshot, retrievalSnapshotPatch } from './retrieval-snapshot.js?v=0.15.0-testing.22';
import { createBackgroundScheduler } from './background-scheduler.js';
import { nextChroniclePromotion } from './chronicle.js';
import { buildPlanningEvidence, createContinuityContextBridge } from './context-bridge.js?v=0.15.0-testing.22';

import { resolveRetrievalAssist } from './retrieval-assist.js?v=0.15.0-testing.22';
import { expandRetrievalTerms } from './semantic-retrieval.js?v=0.15.0-testing.22';
    ...
    while (true) {
        ...
        try {
            ...
            if (failures >= maxFailures) {
                updateRuntime({
                    status: 'error',
                    lastError: `Automatic memory update stopped after ${failures} temporary failures. Pending messages were kept for a later manual retry.`,
                    retryStatus: 'Automatic memory update stopped to limit repeated API requests.',
                });
                return;
            }
...
async function prepareRoleplayGeneration(type) {
    ...
    const settings = getSettings();
    const recentLimit = Math.min(50, Math.max(2, Number(settings.retrievalQueryMessages) || 6));
...
async function performInjectionRefresh(useRetrievalAssist, coverageMessages, recentMessages, promptOptions, refreshRevision) {
    ...
    const availableRecent = Array.isArray(recentMessages)
        ? recentMessages
        : (getContext().chat || []).filter(message => !message?.is_system);
    const queryMessageLimit = Math.min(50, Math.max(2, Number(settings.retrievalQueryMessages) || 6));
    const recent = availableRecent.slice(-queryMessageLimit);
    const { ranks: semanticRanks, terms: expandedTerms, assist: retrievalAssist } = await resolveRetrievalAssist({
        mode: settings.retrievalMode, phase, world, messages: recent, query: queryEmbeddingMemory, expand: expandRetrievalTerms,
    });
    if (!refreshIsCurrent()) return;
    ...
    const injectionStatus = prompt
        ? `Ready to inject approximately ${estimatedTokens} tokens via ${placementStatus} (${budget.mode} recall target: ${budget.tokens} tokens${settings.storySoFarEnabled ? ' + complete active Chronicle frontier' : ''}).`
    ...
    continuityContextBridge.publish(prompt, {
        ...
        planningEvidence: buildPlanningEvidence(world, getChatKey(), coverage, { invalidSourceRanges }),
    });
}
...
async function init() {
    const templateResponse = await fetch(new URL('./settings.html?v=0.15.0-testing.22', import.meta.url));
    ...
    if (event_types.GENERATION_STOPPED) {
        eventSource.on(event_types.GENERATION_STOPPED, () => {
            // ST Stop belongs to the visible reply. Only CM's own controls
            // stop detached memory jobs; saved and pending work stay intact.
            if (!runtime.paused) backgroundMemoryWork.schedule(0);
        });
    }
