...
import { upgradeOocMetaAuthorityPrompt } from './prompts.js?v=0.15.0-testing.17';
...
import { applyReviewBeforeCommitDefault, DEFAULT_REVIEW_BEFORE_COMMIT } from './review-policy.js?v=0.15.0-testing.17';
import { retainLatestPromptRule } from './prompt-migration.js?v=0.15.0-testing.17';
...
const DEFAULTS = Object.freeze({
    ...
    rawTailValue: 0,
    ...PROMPT_DEFAULTS,
    oocMetaAuthorityPromptVersion: 3,
...
export function getSettings() {
    ...
    if (Number(settings.oocMetaAuthorityPromptVersion || 0) < 3) {
        ...
        settings.extractionSystemPrompt = prompt.includes(OOC_META_AUTHORITY_RULE)
            ? prompt
            : prompt.includes(PRE_STRICT_OOC_META_AUTHORITY_RULE)
                ? prompt.replace(PRE_STRICT_OOC_META_AUTHORITY_RULE, OOC_META_AUTHORITY_RULE)
                : `${prompt}\n${OOC_META_AUTHORITY_RULE}`;
        settings.oocMetaAuthorityPromptVersion = 3;
    ...
    if (Number(settings.retrievalDefaultVersion || 0) < 3) {
        if (!['local', 'ai-expanded', 'embedding-hybrid'].includes(settings.retrievalMode)) settings.retrievalMode = 'local';
        settings.retrievalDefaultVersion = 3;
        saveSettingsDebounced();
    }
    ...
    if (Number(settings.retrievalImmediacyPromptVersion || 0) < 1) {
        const previousDefaults = [
            'Expand a roleplay or simulation memory query. Return only {"terms":["..."]} with at most 20 short synonyms, aliases, roles, related actors, concepts, or paraphrases. Never answer the conversation.',
            'Expand a roleplay or simulation memory query by meaning, not by a fixed cue-word list. Resolve short guidance, pronouns, implied participants, time references, and intent from the supplied conversation. Include concise subject, action, object, aliases, synonyms, related actors, concepts, paraphrases, and equivalent wording in the conversation\'s main language when the latest query uses another language. Return only {"terms":["..."]} with at most 20 short terms. Never answer the conversation.',
        ];
        if (!settings.retrievalSystemPrompt || previousDefaults.includes(settings.retrievalSystemPrompt)) {
            settings.retrievalSystemPrompt = PROMPT_DEFAULTS.retrievalSystemPrompt;
        }
    ...
    if (Number(settings.removeStyleDirectiveVersion || 0) < 1) {
        ...
        settings.removeStyleDirectiveVersion = 1;
        saveSettingsDebounced();
    }
    ...
    if (Number(settings.characterProfilePromptVersion || 0) < 3) {
        ...
        let prompt = String(settings.extractionSystemPrompt || PROMPT_DEFAULTS.extractionSystemPrompt);
        if (!prompt.includes(CHARACTER_PROFILE_RULE)) {
    ...
    if (Number(settings.thirdPersonPromptVersion || 0) < 1) {
        ...
        settings.thirdPersonPromptVersion = 1;
        saveSettingsDebounced();
    }
    ...
    if (Number(settings.epistemicPromptVersion || 0) < 5) {
        let extractionPrompt = String(settings.extractionSystemPrompt || PROMPT_DEFAULTS.extractionSystemPrompt);
        if (extractionPrompt.includes(EPISTEMIC_MEMORY_RULES)) {
            // Already current, including on a new installation.
        } else if (extractionPrompt.includes(PRE_STRUCTURED_KNOWLEDGE_BOUNDARY_RULES)) {
            extractionPrompt = extractionPrompt.replace(PRE_STRUCTURED_KNOWLEDGE_BOUNDARY_RULES, EPISTEMIC_MEMORY_RULES);
        } else {
            extractionPrompt = `${extractionPrompt}\n${EPISTEMIC_MEMORY_RULES}`;
        }
        settings.extractionSystemPrompt = extractionPrompt;
        if (settings.injectionInstruction === PRE_KNOWLEDGE_BOUNDARY_INJECTION_INSTRUCTION) {
            settings.injectionInstruction = PROMPT_DEFAULTS.injectionInstruction;
        }
        settings.epistemicPromptVersion = 5;
        saveSettingsDebounced();
    }
    if (Number(settings.epistemicPromptVersion || 0) < 6) {
        let extractionPrompt = String(settings.extractionSystemPrompt || PROMPT_DEFAULTS.extractionSystemPrompt);
        if (extractionPrompt.includes(EPISTEMIC_MEMORY_RULES)) {
            // Already current, including on a new installation.
        } else if (extractionPrompt.includes(PRE_MEMBERSHIP_DISTINCTION_EPISTEMIC_MEMORY_RULES)) {
            extractionPrompt = extractionPrompt.replace(PRE_MEMBERSHIP_DISTINCTION_EPISTEMIC_MEMORY_RULES, EPISTEMIC_MEMORY_RULES);
        } else {
            extractionPrompt = `${extractionPrompt}\n${EPISTEMIC_MEMORY_RULES}`;
        }
        settings.extractionSystemPrompt = extractionPrompt;
        settings.epistemicPromptVersion = 6;
        saveSettingsDebounced();
    }
    ...
    }
    {
        ...
        let compactedPrompt = retainLatestPromptRule(
            ...
            [
                ...
                'Keep objective canon separate from subjective perspective.',
