...
import { applyReviewBeforeCommitDefault, DEFAULT_REVIEW_BEFORE_COMMIT } from './review-policy.js?v=0.14.0-standalone.258';
import { retainLatestPromptRule } from './prompt-migration.js?v=0.14.0-standalone.258';
...
const DEFAULTS = Object.freeze({
    ...
    rawTailValue: 0,
    ...PROMPT_DEFAULTS,
    oocMetaAuthorityPromptVersion: 1,
    chatWorlds: {},
    deletedWorldIds: [],
});
...
export function getSettings() {
    ...
    if (Number(settings.oocMetaAuthorityPromptVersion || 0) < 1) {
        const prompt = String(settings.extractionSystemPrompt || PROMPT_DEFAULTS.extractionSystemPrompt);
        settings.extractionSystemPrompt = prompt.includes(OOC_META_AUTHORITY_RULE)
            ? prompt
            : `${prompt}\n${OOC_META_AUTHORITY_RULE}`;
        settings.oocMetaAuthorityPromptVersion = 1;
