...
export function rankSuperiorSyncedWorlds(worlds, currentWorld, { characterName, chatId } = {}) {
    ...
    const expectedName = `${String(characterName || 'Chat')} · ${String(chatId || 'Memory')}`;
    const currentDigest = Array.isArray(currentWorld.extractions) ? currentWorld.extractions.length : 0;
    return (Array.isArray(worlds) ? worlds : [])
        .filter(world => world?.id && world.id !== currentWorld.id && world.name === expectedName)
        .filter(world => Math.max(0, Number(world.counts?.retryableDigest) || 0) > currentDigest)
        .sort((left, right) => (Number(right.counts?.retryableDigest) || 0) - (Number(left.counts?.retryableDigest) || 0)
