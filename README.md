...
Detached Digest and Chronicle jobs keep completed chunks in server storage when the browser disconnects. Status polling reconnects to the same job; returning to the tab or regaining connectivity refreshes saved progress and schedules pending automatic work. A temporary startup connection failure does not switch storage backends. SillyTavern's reply Stop button leaves memory processing alone; Continuity's own Stop/Pause controls still stop it. The SillyTavern server must remain running, and model calls still require a working provider connection.
