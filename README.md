...
Retrieval supplements the default story-so-far overview rather than replacing it. Story so far is a separate persisted rolling world-state snapshot, generated only by its selected model from its previous version plus new chronological source material. By default, its source is completed compact L1 scene summaries only; newer uncovered chat remains verbatim in SillyTavern until L1 processes it. Raw chat remains an optional source mode. Story waits when an expected L1 is missing rather than silently replacing it with raw history. It organizes foundational premise, chronological major developments, durable state at the covered boundary, and unresolved central matters without fixed item or character limits. The total Story allowance is the only size cap: its cross-tokenizer safety band adapts automatically from 68–80% at small allowances toward 76–88% at the 6,000-token automatic maximum, allowing larger snapshots to use proportionally more space without making small caps fragile. Any remaining over-budget candidate is automatically condensed through progressively safer complete rewrites, never clipped, truncated, saved partially, or exposed as a failure after only the first oversized answer. Normal builds, rebuilds, and automatic updates use one successful model call per chunk. The separate manual Refine action performs the slower factual review against the covered source and invokes a full repair and recheck only when it detects a material issue; refinement never runs automatically. An ordinary completed scene normally needs one outcome-level development, and an excerpt that changes no load-bearing continuity may leave the snapshot unchanged. Story never reads L2, L3, retrieved records, or other recall output. A fresh or intentional rebuild packs as much selected history as safely fits each request; later automatic updates use the configured message cadence. Story stores bounded, hidden, fingerprinted recovery checkpoints every 32 covered messages. An edit or deletion rewinds to the newest checkpoint whose complete prefix still matches raw chat, discards every contaminated later checkpoint, and rebuilds forward automatically; if none is safe, it rebuilds from the beginning. These checkpoints consume storage only and are never injected. Transient API failures retry with backoff. Build / Continue preserves completed work, advances a completed Story, and resumes a stopped or failed operation; Rebuild immediately deletes the current Story and starts again from the first eligible selected source; Refine audits the stored Story without rebuilding its source range; Delete removes only Story so far. Changing source mode requires one safe rebuild and never alters structured memory. The main Build action also builds or continues Story so far, and Erase everything & start over reconstructs it after structured memory. Its automatic additive allowance uses 2% of model context with a 1,500-token minimum and 6,000-token maximum, so enabling it does not reduce recall. Popup messages report both completed work and the already-current result when enabled. Automatic recall uses 20% of model context; fixed overrides remain available for both lanes.
